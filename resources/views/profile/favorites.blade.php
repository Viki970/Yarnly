...
<section class="relative overflow-hidden bg-gradient-to-br from-pink-50 via-rose-50 to-red-50 py-16 dark:from-zinc-900 dark:via-gray-900 dark:to-black">
    ...
    <div class="relative max-w-6xl mx-auto px-6 lg:px-12">
        ...
        <div class="mt-6 flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
            <div class="max-w-3xl">
                <h1 class="text-4xl font-bold tracking-tight text-pink-900 sm:text-5xl dark:text-pink-100">{{ __('Your favorite patterns') }}</h1>
                <p class="mt-4 text-lg leading-relaxed text-zinc-600 dark:text-zinc-400">
                    {{ __('All the crochet patterns you\'ve saved for later. Your personal collection of inspiration and future projects.') }}
                </p>
            </div>
            <div class="rounded-2xl bg-white/80 p-6 shadow-xl ring-1 ring-pink-100 backdrop-blur dark:bg-zinc-800/60 dark:ring-pink-500/50">
                <div class="text-center">
                    <p class="text-sm font-medium text-pink-400 dark:text-pink-300">{{ $tab == 'patterns' ? __('Saved patterns') : __('Saved collections') }}</p>
                    <p class="mt-2 text-3xl font-bold text-pink-900 dark:text-white">{{ $tab == 'patterns' ? $totalPatterns : $totalCollections }}</p>
                    <p class="mt-1 text-sm text-zinc-600 dark:text-zinc-400">{{ __('Ready to create') }}</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="max-w-6xl mx-auto px-6 lg:px-12">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
            <div class="bg-white dark:bg-gray-800/70 rounded-2xl p-6 shadow-lg border border-pink-100 dark:border-pink-500/40">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        <p class="text-sm font-medium text-zinc-600 dark:text-zinc-400">{{ $tab == 'patterns' ? __('Total Patterns') : __('Total Collections') }}</p>
        ...
        @if(($tab == 'patterns' && $totalPatterns > 0) || ($tab == 'collections' && $totalCollections > 0))
            <div class="mb-8 bg-gradient-to-br from-pink-50 via-rose-50 to-red-50 dark:from-gray-800/70 dark:via-gray-800/70 dark:to-gray-800/70 rounded-2xl p-6 shadow-sm border border-pink-100 dark:border-pink-500/40">
                <h3 class="text-sm font-semibold text-pink-900 dark:text-pink-200 mb-4">{{ __('Filter by craft type:') }}</h3>
                <div class="flex flex-wrap gap-3">
                    <a href="{{ route('patterns.favorites', ['tab' => $tab, 'filter' => 'all']) }}" 
                       class="px-6 py-3 rounded-full text-sm font-semibold transition-all duration-300 ease-in-out {{ $craftFilter == 'all' ? 'bg-gradient-to-r from-pink-300 to-pink-400 text-white shadow-md scale-105' : 'bg-white dark:bg-gray-700/50 text-zinc-700 dark:text-zinc-300 hover:bg-gradient-to-r hover:from-pink-50 hover:to-rose-50 dark:hover:from-pink-500/20 dark:hover:to-pink-400/20 hover:text-pink-700 dark:hover:text-pink-300 hover:shadow-sm hover:scale-105' }}">
                        {{ __('All') }} {{ $tab == 'patterns' ? __('Patterns') : __('Collections') }}
                    </a>
        ...
        <div class="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
            <div>
                <p class="text-sm font-semibold text-pink-700 dark:text-pink-200">{{ __('Your collection') }}</p>
                <h2 class="text-3xl font-bold text-zinc-900 dark:text-white">{{ __('Favorite patterns') }}</h2>
                <p class="mt-2 text-sm text-zinc-600 dark:text-zinc-300">{{ __('Patterns you\'ve saved to create later') }}</p>
            </div>
            <a href="{{ route('patterns.crochet') }}" class="text-sm font-semibold text-zinc-600 underline-offset-4 hover:underline dark:text-zinc-300">{{ __('Browse more patterns') }}</a>
        </div>
                    ...
                    <article class="group rounded-2xl border border-pink-100 bg-white p-5 shadow-sm transition hover:-translate-y-1 hover:shadow-lg dark:border-pink-500/30 dark:bg-gray-800/60">
                        ...
                        @if($pattern->pdf_file)
                            <div class="mt-5 flex gap-2">
                                <a href="{{ route('patterns.view', $pattern->id) }}" class="flex-1 rounded-lg bg-pink-500 px-4 py-2 text-center text-sm font-semibold text-white transition hover:bg-pink-600">{{ __('View Pattern') }}</a>
                                <a href="{{ route('patterns.download', $pattern) }}" class="flex-1 rounded-lg bg-pink-300 px-4 py-2 text-center text-sm font-semibold text-white transition hover:bg-pink-400">{{ __('Download PDF') }}</a>
                            </div>
                        @else
                            <button disabled class="mt-5 block w-full rounded-lg bg-zinc-200 px-4 py-2 text-center text-sm font-semibold text-zinc-500 cursor-not-allowed dark:bg-zinc-600/30 dark:text-zinc-400">{{ __('PDF Coming Soon') }}</button>
                        @endif
        ...
        @else
            <div class="mt-12 rounded-2xl border border-dashed border-pink-200 bg-pink-50 p-12 text-center dark:border-pink-500/40 dark:bg-gray-800/60">
                ...
                <p class="mt-2 text-sm text-pink-700 dark:text-pink-300">{{ __('Start exploring patterns and save your favorites by clicking the heart icon!') }}</p>
                <a href="{{ route('patterns.crochet') }}" class="mt-6 inline-block rounded-lg bg-pink-300 px-6 py-3 text-sm font-semibold text-white transition hover:bg-pink-400">{{ __('Browse Patterns') }}</a>
            </div>
        @endif
        @else
        <!-- Collections Section -->
        <div class="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between mb-8">
            <div>
                <p class="text-sm font-semibold text-pink-700 dark:text-pink-200">{{ __('Your collection') }}</p>
                <h2 class="text-3xl font-bold text-zinc-900 dark:text-white">{{ __('Favorite collections') }}</h2>
                <p class="mt-2 text-sm text-zinc-600 dark:text-zinc-300">{{ __('Collections you\'ve saved for inspiration') }}</p>
            </div>
            <a href="{{ route('my-collections') }}" class="text-sm font-semibold text-zinc-600 underline-offset-4 hover:underline dark:text-zinc-300">{{ __('Browse more collections') }}</a>
        </div>
        ...
        @else
            <div class="mt-12 rounded-2xl border border-dashed border-pink-200 bg-pink-50 p-12 text-center dark:border-pink-500/40 dark:bg-gray-800/60">
                ...
                <p class="mt-2 text-sm text-pink-700 dark:text-pink-300">{{ __('Discover collections created by other makers and save your favorites!') }}</p>
                <a href="{{ route('my-collections') }}" class="mt-6 inline-block rounded-lg bg-pink-300 px-6 py-3 text-sm font-semibold text-white transition hover:bg-pink-400">{{ __('Browse Collections') }}</a>
            </div>
        @endif
        @endif
    </div>
